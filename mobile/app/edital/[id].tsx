...
export default function TelaDetalheEdital() {
  ...
  useEffect(() => {
    const buscarDetalhes = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_URL}/editais/${id}`);
      ...
      <View style={styles.content}>
        ...
        <View style={styles.priceCard}>
          ...
          <Text style={styles.priceValue}>
            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(edital.valor_total_estimado)}
          </Text>
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Informações do Órgão</Text>
          <Text style={styles.infoText}><Text style={styles.bold}>Instituição:</Text> {edital.orgao_razao_social}</Text>
          <Text style={styles.infoText}><Text style={styles.bold}>CNPJ:</Text> {edital.orgao_cnpj}</Text>
          <Text style={styles.infoText}><Text style={styles.bold}>Localização:</Text> {edital.municipio} - {edital.uf}</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.dateRow}>
            <View>
              ...
              <Text style={styles.dateValue}>{new Date(edital.data_abertura_proposta).toLocaleDateString('pt-BR')}</Text>
            </View>
            <View>
              <Text style={styles.dateLabel}>Encerramento</Text>
              <Text style={styles.dateValue}>{new Date(edital.data_encerramento_proposta).toLocaleDateString('pt-BR')}</Text>
            </View>
          </View>
        </View>

        <Text style={styles.footerNote}>Nº Controle PNCP: {edital.numero_controle_pncp}</Text>

        <TouchableOpacity 
          style={styles.button}
          onPress={() => Linking.openURL(`https://pncp.gov.br/app/editais/${edital.numero_controle_pncp}`)}
        >
...
const styles = StyleSheet.create({
  ...
  statusBadge: { backgroundColor: '#E1F5FE', color: '#01579B', padding: 5, borderRadius: 5, fontSize: 12, fontWeight: 'bold' },
  modalidadeBadge: { backgroundColor: '#F3E5F5', color: '#4A148C', padding: 5, borderRadius: 5, fontSize: 12, fontWeight: 'bold' },
  label: { fontSize: 12, color: '#8E8E93', textTransform: 'uppercase', marginBottom: 5 },
  titulo: { fontSize: 20, fontWeight: 'bold', color: '#1C1C1E', marginBottom: 20 },
  priceCard: { backgroundColor: '#F2F2F7', padding: 15, borderRadius: 10, marginBottom: 25 },
  priceLabel: { fontSize: 14, color: '#3A3A3C', marginBottom: 5 },
  priceValue: { fontSize: 24, fontWeight: 'bold', color: '#34C759' },
  section: { marginBottom: 25, borderBottomWidth: 1, borderBottomColor: '#F2F2F7', paddingBottom: 15 },
  ...
  footerNote: { fontSize: 12, color: '#C7C7CC', textAlign: 'center', marginTop: 10 },
